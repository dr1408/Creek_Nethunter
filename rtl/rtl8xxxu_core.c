...
#include "rtl8188fufw_fw.h"
#include "rtl8188eufw_fw.h"
...
int rtl8xxxu_load_firmware(struct rtl8xxxu_priv *priv, const char *fw_name)
{
	...
	if (strstr(fw_name, "rtl8188fufw")) {
		/* embedded firmware: avoids filesystem/SELinux firmware load */
		fwdata = rtl8188fufw_data;
		fwlen = rtl8188fufw_len;
		dev_info(dev, "%s: using embedded RTL8188FU firmware (%zu bytes)\n",
			 DRIVER_NAME, fwlen);
	} else if (strstr(fw_name, "rtl8188eufw")) {
		/* embedded firmware: avoids filesystem/SELinux firmware load */
		fwdata = rtl8188eufw_data;
		fwlen = rtl8188eufw_len;
		dev_info(dev, "%s: using embedded RTL8188EU firmware (%zu bytes)\n",
			 DRIVER_NAME, fwlen);
	} else {
	...
	priv->fw_data = kmemdup(fwdata, fwlen, GFP_KERNEL);
	if (!priv->fw_data) {
		ret = -ENOMEM;
		goto exit;
	}
	priv->fw_size = fwlen - sizeof(struct rtl8xxxu_firmware_header);
